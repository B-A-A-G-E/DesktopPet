...
class PetWindow(QWidget):
    def __init__(self):
        ...
        # 入场并切换待机
        self.replyAction("entre", False, False)
        self.stateMenu.log("Succeed to entre", LogType.Entre)
        self.replyAction("idle")
    ...
    def bind(self) -> None:
        ...
        self.idleTimer.timeout.connect(lambda: self.replyAction("idle"))
    ...
    def replyAction(self, action: str, afterEvent: bool = False, isAsync: bool = True) -> None:
        """
        执行对应行动时进行响应\n
        若afterEvent为True，则先回应当前状态的after-event事件(动画只能同步播放)\n
        """
        if self.state != action or (action == "idle" and self.state == "idle"):
            # 切换计时器状态
            if action != "idle":
                self.idleTimer.stop()
                self.moveTimer.stop()
                ...
                if not self.moveTimer.isActive():
                    self.moveTimer.start(data.base["idle-move-time"])
            # 在dialogMenu回复
            self.dialogMenu.addLine(conv.replyText("action", action))
            # 切换动画
            self.currentAnime.over()
            if afterEvent and f"after-{self.state}" in self.animes.keys():
                self.replyAction(f"after-{self.state}", False, False)
            self.currentAnime = self.animes[action]
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        if event.buttons() == Qt.MouseButton.LeftButton:
            ...
            if self.state == "idle" and collision:
                event.accept()
                self.replyAction(collision)
            else:
                self.move(event.globalPosition().toPoint() - self.dragPosition)
                event.accept()
                self.replyAction("drag")
    
    def mouseReleaseEvent(self, event: QMouseEvent):
        if self.state != "idle":
            self.replyAction("idle", True)
