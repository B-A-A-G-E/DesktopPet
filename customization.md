...
class Action(Plugin):
    def __init__(self):
        ...
        self.name = "Action"          # 在行动面板显示的名称
        self.description = "This is an action"  # 可选，用于行动面板的鼠标悬浮提示
...
调用 `self.window.changeAnime(self.id)`以播放。
...
然后在 `start` 中调用 `self.window.dialogMenu.addLine` 手动显示。
...
class Dance(Plugin):
    ...
    def start(self):
        ...
        # 显示回复
        self.window.dialogMenu.addLine("来跳舞吧！")
...
> **注意**：
> - ~~安装事件过滤器后，需在 `teardown` 中卸载~~（基类已处理，无需额外操作）
